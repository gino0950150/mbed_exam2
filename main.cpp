...
void GestureUI_STOP(Arguments *in, Reply *out);
RPCFunction rpcGestureUI_STOP(&GestureUI_STOP, "GestureUI_STOP");

void Return_features(Arguments *in, Reply *out);
RPCFunction rpcReturn_features(&Return_features, "Return_features");
...
//     sprintf(buff, "QoS0 Hello, Python! #%d", message_num);
    ...
    message.dup = false;
    message.payload = (void*) feature;
    message.payloadlen = strlen(feature) + 1;
    // char angle_topic[] = "M";
    int rc = client_out->publish("features", message);
...
void publish_GestureID_message() {
    message_num++;
    MQTT::Message message;
    char buff[200];
    sprintf(buff, "%d", angle);
...
void analyze_feature(float avg) {
    if(z >= 20) return;
    if(avg >= 0.0)
        feature[z] = '2';
    else
...
int GestureUI() {
    ...
    while (true) {
        ...
        // error_reporter->Report(config.output_message[gesture_index]);
            ...
            refresh_uLCD();
            publish_GestureID_message();
            analyze_feature(got_data);
        }
    }
}
